/*
 *  ласс ответственный за непосредственное создание структуры
 */
...
public class msSqlDate {
	...
	public void connect() {//коннектимс€
	...
	public void createStructure(ArrayList<emitets> date){//создание таблиц с названием эмитетов и их ID
		...
		try {
			pstmt = con.prepareStatement("{call CreateEmitetsStructure()}");//—оздание сводной таблицы
			rs = pstmt.execute(); 
	...
	public void InsertDate(String nameTable, List<String[]> date) throws SQLException, NumberFormatException {//функци€ добавлени€ котировок в таблицы.
		PreparedStatement pstmt;
			...
			System.out.println("ƒобавлено: " + i);
	}
}
